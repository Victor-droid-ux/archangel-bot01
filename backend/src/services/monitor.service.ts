...
type Position = {
  ...
  netSol: number; // net SOL exposure/value (from aggregation)
  avgBuyPrice?: number; // SOL per token (approx)
  // optional per-position overrides (future-proof)
...
export function startPositionMonitor(
  ...
  const tick = async () => {
    try {
      const positions: Position[] = await dbService.getPositions();
