...
export async function startTokenWatcher(io?: any) {
  const intervalMs = Number(
    process.env.TOKEN_WATCH_INTERVAL_MS ?? ENV.TOKEN_WATCH_INTERVAL_MS ?? 10_000
  );
  ...
  try {
    ...
    for (const t of initial)
      seen.add(t.mint ?? (t as any).address ?? (t as any).token ?? t.symbol);
  } catch (e) {
