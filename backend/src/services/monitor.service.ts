...
const DEFAULT_TP = Number(
  process.env.DEFAULT_TAKE_PROFIT_PCT ?? ENV.DEFAULT_TAKE_PROFIT_PCT ?? 0.1
); // 10%
const DEFAULT_SL = Number(
  process.env.DEFAULT_STOP_LOSS_PCT ?? ENV.DEFAULT_STOP_LOSS_PCT ?? 0.05
); // 5%

export async function startPositionMonitor(io?: any) {
  ...
  async function check() {
    try {
      ...
      for (const p of tokenPrices || [])
        priceMap.set(
          p.mint ?? (p as any).address ?? p.symbol,
          Number(p.price ?? 0)
        );
      ...
      for (const pos of positions) {
        try {
          ...
          if (pnlPct >= TP || pnlPct <= -SL) {
            log.info(
              `Position ${mint} hit threshold. pnl=${(pnlPct * 100).toFixed(
                2
              )}% -> auto-sell.`
            );
            // auto sell position
            ...
            if (sellRes?.success) {
              ...
              const errInfo =
                (sellRes as any)?.data?.error ??
                sellRes?.data ??
                sellRes ??
                "unknown error";
              log.warn(
                "Auto-sell failed for " + mint + " : " + (typeof errInfo === "string" ? errInfo : JSON.stringify(errInfo))
              );
            }
          }
        } catch (innerErr: any) {
          log.error(
            "Error checking position: " + (innerErr?.message ?? innerErr)
          );
        }
      }
    } catch (err: any) {
