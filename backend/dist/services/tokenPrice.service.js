...
const TOKEN_LIST_URL = "https://raw.githubusercontent.com/jup-ag/token-list/main/src/tokens/mainnet.json";
...
export async function initTokenPriceService(io) {
    ...
    setInterval(() => {
        refreshPricesAndBroadcast(io).catch((err) => log.error("Periodic refresh failed", err.message));
    }, 10_000);
}
...
async function loadTokenList() {
    ...
    try {
        ...
        tokenList = Object.values(data).filter((t) => t?.address && t?.symbol);
        log.info(`✅ Loaded ${tokenList.length} Jupiter tokens`);
    }
    catch (err) {
        log.error({ err: err.message }, "❌ Unable to load Jupiter token list - enabling SAFE FALLBACK LIST");
...
async function refreshPricesAndBroadcast(io) {
    try {
        const response = await axios.get("https://public-api.birdeye.so/public/market/price?address=So11111111111111111111111111111111111111112", {
            headers: {
                "x-chain": "solana",
            },
        });
