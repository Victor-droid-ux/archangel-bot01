 ...
 */
export async function executeJupiterSwap({ inputMint, outputMint, amount, userPublicKey, slippage = 1, }) {
    try {
        ...
        const quoteResponse = await getJupiterQuote(inputMint, outputMint, amount, slippage);
        if (!quoteResponse)
            throw new Error("Failed to get Jupiter quote");
        // 🔹 Request swap transaction from Jupiter API
        const { data } = await axios.post(`${JUPITER_SWAP}/swap`, {
            quoteResponse,
            userPublicKey,
