  ...
  try {
    ...
    const msg =
      err && typeof err === "object" && "message" in err
        ...
        : String(err);
    log.warn(
      `fetchJupiterTokenList failed, fallback to jup token list: ${msg}`
    );
...
export function startTokenWatcher(io: Server, opts?: { intervalMs?: number }) {
  ...
  log.info(
    `Starting token watcher intervalMs=${intervalMs} MIN_MC_SOL=${MIN_MC_SOL}`
  );
  ...
  const tick = async () => {
    try {
      ...
      const candidates: CandidateToken[] = tokensArray
        .slice(0, 500)
        .map((t: any) => ({
          symbol: t.symbol,
      ...
      for (const tk of candidates) {
        ...
        if (!currentMints.has(tk.mint)) {
          ...
          if (mc >= MIN_MC_SOL) {
            ...
            log.info(
              `Candidate meets min MC mint=${tk.mint} symbol=${tk.symbol} marketCapSol=${mc}`
            );
            ...
            registerAutoBuyCandidate(io, tk).catch((e: any) => {
              const emsg =
                e && typeof e === "object" && "message" in e
                  ...
                  : String(e);
              log.warn(
                `candidate did not meet MC but attempted registration: ${emsg}`
              );
            });
          }
        }
