    ...
    try {
        ...
        try {
            ...
            if (Array.isArray(prices) && prices.length > 0) {
                ...
                log.info(`✅ Built token list from Jupiter price endpoint (${tokenList.length} tokens)`);
                return;
            }
        }
        catch (err2) {
            log.error({ err: err2?.message }, "❌ Fallback via fetchTokenPrices failed");
        }
    ...
    for (let i = 0; i < allMints.length; i += chunkSize) {
        ...
        try {
            ...
            log.error({
                ...
                data: err.response?.data,
                url: reqUrl,
            }, `❌ Price chunk fetch failed (mints ${i}-${i + chunkSize}): ${err.message}`);
        }
    }
